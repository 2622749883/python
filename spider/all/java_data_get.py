...
def parse_detail_page(html,headers):
    ...
    for item in items_detail:
        ...
        try:
            sql = "insert into java(job_name,company_name,salary,job_request,address,education_requirements) values ('"+item[1]+"','"+item[2]+"','"+item[3]+"','"+content[0]+"','"+address[0]+"','"+education[0]+"');"
            cursor.execute(sql)
            conn.commit()
        except:
            print("数据插入异常............")
...
def main():
    ...
    for i in range(1435):
        url = "https://search.51job.com/list/000000,000000,0000,00,9,99,java,2,"+str(i+1)+".html?lang=c&postchannel=0000&workyear=99&cotype=99&degreefrom=99&jobterm=99&companysize=99&ord_field=0&dibiaoid=0&line=&welfare="
        try:
